...
namespace Opsive.UltimateCharacterController.Editor.Inspectors.Character
{
    ...
    public class CharacterLocomotionInspector : UIStateBehaviorInspector
    {
        ...
        protected override void ShowHeaderElements(VisualElement container)
        {
            ...
            FieldInspectorView.AddField(target, target, "m_TimeScale", foldout, (object o) =>
            {
                EditorUtility.SetDirty(target);
                var timeScale = characterLocomotion.TimeScale;
                characterLocomotion.TimeScale = -1;
                characterLocomotion.TimeScale = timeScale; // Set the property so the event is executed.
            });
            ...
            FieldInspectorView.AddField(target, "m_MovingPlatformDisconnectMovementMultiplier", foldout);
