...
namespace Opsive.UltimateCharacterController.Utility.Builders
{
    ...
    public static class ViewTypeBuilder
    {
        ...
        public static ViewType AddViewType(CameraController cameraController, Type viewType)
        {
            ...
            if (viewTypeObj is FirstPersonController.Camera.ViewTypes.FirstPerson firstPersonViewType) {
                var activeRenderPipeline = UnityEngineUtility.GetActiveRenderPipeline();
                if (activeRenderPipeline == UnityEngineUtility.RenderPipeline.URP || activeRenderPipeline == UnityEngineUtility.RenderPipeline.HDRP) {
                    firstPersonViewType.OverlayRenderType = FirstPersonController.Camera.ViewTypes.FirstPerson.ObjectOverlayRenderType.RenderPipeline;
                } else {
                    AddFirstPersonCamera(cameraController, firstPersonViewType);
                }
            }
#endif
        ...
        public static void AddFirstPersonCamera(CameraController cameraController, FirstPersonController.Camera.ViewTypes.FirstPerson viewType)
        {
            ...
            if (firstPersonCamera == null) {
                UnityEngine.Transform cameraControllerTransform;
                UnityEngine.Transform firstPersonCameraTransform;
#if UNITY_EDITOR
                var isPrefab = UnityEditor.PrefabUtility.IsPartOfAnyPrefab(cameraController);
                if (isPrefab) {
                    var prefabScope = new UnityEditor.PrefabUtility.EditPrefabContentsScope(UnityEditor.AssetDatabase.GetAssetPath(cameraController));
                    cameraControllerTransform = prefabScope.prefabContentsRoot.GetComponentInChildren<CameraController>(true).transform;
                } else {
#endif
                    cameraControllerTransform = cameraController.transform;
#if UNITY_EDITOR
                }
#endif

                if ((firstPersonCameraTransform = cameraControllerTransform.Find("FirstPersonCamera")) != null || (firstPersonCameraTransform = cameraControllerTransform.Find("First Person Camera")) != null) {
                    firstPersonCamera = firstPersonCameraTransform.GetComponent<UnityEngine.Camera>();
                }

                if (firstPersonCamera == null) {
                    var cameraGameObject = new UnityEngine.GameObject("FirstPersonCamera");
                    cameraGameObject.transform.SetParentOrigin(cameraControllerTransform);
#if UNITY_EDITOR
                    if (isPrefab) {
                        cameraGameObject = cameraControllerTransform.Find("FirstPersonCamera").gameObject;
                    }
#endif
