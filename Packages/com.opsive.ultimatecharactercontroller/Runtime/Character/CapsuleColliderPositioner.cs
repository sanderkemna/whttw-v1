...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CapsuleColliderPositioner : StateBehavior
    {
        ...
        private void AdjustRadiusAdjustment(float targetOffset)
        {
            ...
            var collisionLayerEnabled = m_CharacterLocomotion.CollisionLayerEnabled;
            m_CharacterLocomotion.EnableColliderCollisionLayer(false);
            var firstEndCap = m_Transform.TransformPoint(Vector3.up * m_CapsuleCollider.radius);
            var secondEndCap = firstEndCap + m_Transform.TransformDirection(Vector3.up * m_CapsuleCollider.height);
            if (Physics.OverlapCapsuleNonAlloc(firstEndCap, secondEndCap, m_CapsuleCollider.radius - m_CharacterLocomotion.ColliderSpacing,
                                    m_OverlapColliders, m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore) > 0) {
                m_CapsuleCollider.radius -= delta;
