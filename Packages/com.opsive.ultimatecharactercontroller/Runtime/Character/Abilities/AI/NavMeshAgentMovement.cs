...
namespace Opsive.UltimateCharacterController.Character.Abilities.AI
{
    ...
    public class NavMeshAgentMovement : PathfindingMovement
    {
        ...
        public override void Update()
        {
            ...
            var lookRotation = m_CharacterLocomotion.Rotation;
            ...
            if (updateInput && m_NavMeshAgent.hasPath && !m_NavMeshAgent.isStopped) {
                ...
                if (m_NavMeshAgent.desiredVelocity.sqrMagnitude > 0 && m_NavMeshAgent.remainingDistance > m_CharacterLocomotion.ColliderSpacing) {
                    Vector3 velocity;
                    if (m_NavMeshAgent.remainingDistance > m_ArrivedDistance &&
                        ...
                        lookRotation = Quaternion.LookRotation(m_NavMeshAgent.desiredVelocity.normalized, m_CharacterLocomotion.Up);
                        // The normalized velocity should be relative to the target rotation.
                        velocity = Quaternion.Inverse(lookRotation) * m_NavMeshAgent.desiredVelocity.normalized;
                    } else {
                        velocity = m_CharacterLocomotion.InverseTransformDirection(m_NavMeshAgent.desiredVelocity);
                    }
            ...
            var rotation = lookRotation * Quaternion.Inverse(m_CharacterLocomotion.Rotation);
        ...
        {
            if (!m_NavMeshAgent.hasPath || m_NavMeshAgent.pathPending || m_NavMeshAgent.remainingDistance > m_CharacterLocomotion.DesiredMovement.magnitude) {
                return;
            }

            // Prevent the character from jittering back and forth to land precisely on the target.
            var direction = m_NavMeshAgent.destination - m_CharacterLocomotion.Position;
            var desiredMovement = direction.normalized * m_CharacterLocomotion.DesiredMovement.magnitude;
            var localDesiredMovement = m_Transform.InverseTransformDirection(desiredMovement);
            localDesiredMovement.y = m_CharacterLocomotion.LocalDesiredMovement.y;
            m_CharacterLocomotion.DesiredMovement = m_Transform.TransformDirection(localDesiredMovement);
        }
        ...
        public override void LateUpdate()
        {
            ...
            m_NavMeshAgent.nextPosition = m_CharacterLocomotion.Position;
        }
        ...
        protected virtual bool UpdateOffMeshLink()
        {
            if (m_NavMeshAgent.currentOffMeshLinkData.linkType == OffMeshLinkType.LinkTypeJumpAcross ||
                ...
                if (direction.sqrMagnitude > 0.1f || m_CharacterLocomotion.Grounded) {
                    var nextPositionDirection = m_CharacterLocomotion.InverseTransformPoint(m_NavMeshAgent.currentOffMeshLinkData.endPos);
        ...
        protected virtual void OnGrounded(bool grounded)
        {
            if (grounded && m_NavMeshAgent.enabled) {
                ...
                m_NavMeshAgent.Warp(m_CharacterLocomotion.Position);
