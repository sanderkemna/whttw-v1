...
#endif
            if (m_ItemSetRules != null) {
                var hasDefault = false;
                for (int i = 0; i < m_ItemSetRules.Length; ++i) {
                    if (m_ItemSetRules[i].IsDefaultItemSetRule()) {
                        if (hasDefault) {
                            Debug.LogWarning($"Warning: multiple Item Sets are set to Default within the {name} rule. " +
                                "This will cause problems when determing the default Item Set to equip.");
                            break;
                        }
                        hasDefault = true;
                    }
