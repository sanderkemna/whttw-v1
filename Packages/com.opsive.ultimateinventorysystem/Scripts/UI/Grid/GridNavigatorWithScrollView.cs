...
namespace Opsive.UltimateInventorySystem.UI.Grid
{
    ...
    public class GridNavigatorWithScrollView : GridNavigatorBase
    {
        ...
        [Tooltip("Hide the scrollbar if there is no scroll")]
        [SerializeField] protected bool m_HideScrollbarIfNoScroll = false;
        ...
        //Add one because there is an extra slot that we hide.
        protected override int MaxSliceIndex => base.MaxSliceIndex + 1;
        ...
        private void HandleGridElementSelected(int index)
        {
            ...
            var halfGridElementSize = m_GridElementSize / 2f;
            if (offset < halfGridElementSize) {
            ...
            } else if (offset > halfGridElementSize) {
        ...
        {
            var indexNormalized1 = m_Index / (float)(MaxSliceIndex + 1);
            var valueNormalized = value;

            var normalizedOffset = valueNormalized - indexNormalized1;
        ...
        public override void RefreshArrows()
        {
            ...
            // Set the scroll index taking into account the anchor offset.

            var offset = m_Vertical ? 
                (m_Content.anchoredPosition.y > MinAnchorPoint.y ? 1 : 0) : 
                (m_Content.anchoredPosition.x > MinAnchorPoint.x ? 1 : 0);
            var indexWithOffset = m_Index + offset;
            m_Scrollbar.Scrollbar.SetValueWithoutNotify(indexWithOffset / (float)(MaxSliceIndex +1));
            
            if (m_HideScrollbarIfNoScroll) {
                var showScrollbar = GridElementCount > m_Grid.GridSizeCount;
                if (m_Scrollbar.gameObject.activeSelf != showScrollbar) {
                    m_Scrollbar.gameObject.SetActive(showScrollbar);
                }
            }
        }
    }
