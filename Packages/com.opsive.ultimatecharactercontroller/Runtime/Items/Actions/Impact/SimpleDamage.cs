...
namespace Opsive.UltimateCharacterController.Items.Actions.Impact
{
    ...
    {
        [Tooltip("Use the impact damage data from the context if it is possible? This can be from an external source like a weapon, a previous impact action in the list, and/or a default value. If true most of the values may be ignored if already defined in the context.")]
        [SerializeField] protected bool m_UseContextData;
        [Tooltip("Set the Damage Impact Data that streams through the impact actions to give context to the next impacte action or function invoked by an impact event.")]
        [SerializeField] protected bool m_SetDamageImpactData = true;
        [Tooltip("Invoke the OnObjectImpact event on the object impacted. Listen to that event if you which to handle special cases per object.")]
