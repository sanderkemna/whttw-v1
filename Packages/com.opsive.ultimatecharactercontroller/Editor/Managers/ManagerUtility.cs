...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public static class ManagerUtility
    {
        ...
        public static Inventory.ItemCollection FindItemCollection(ScriptableObject editorWindow)
        {
            ...
            var itemSetManager = UnityEngine.Object.FindFirstObjectByType<Inventory.ItemSetManager>();
#else
        ...
        /// </summary>
        public static Material FindInvisibleShadowCaster()
        {
            string shadowCasterGUID;
            var activeRenderPipeline = UnityEngineUtility.GetActiveRenderPipeline();
            if (activeRenderPipeline == UnityEngineUtility.RenderPipeline.URP) {
                shadowCasterGUID = c_InvisibleShadowCasterURPGUID;
            } else if (activeRenderPipeline == UnityEngineUtility.RenderPipeline.HDRP) {
                shadowCasterGUID = c_InvisibleShadowCasterHDRPGUID;
            } else {
                shadowCasterGUID = c_InvisibleShadowCasterGUID;
            }

            // The GUID should remain consistant. 
            var shadowCasterPath = AssetDatabase.GUIDToAssetPath(shadowCasterGUID);
