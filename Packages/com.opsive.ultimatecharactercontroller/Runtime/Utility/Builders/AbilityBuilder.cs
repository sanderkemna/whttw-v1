...
namespace Opsive.UltimateCharacterController.Utility.Builders
{
    ...
    public static class AbilityBuilder
    {
        ...
        private static Dictionary<Type, DefaultDetectGroundCollisions> s_DefaultDetectGroundCollisions = new Dictionary<Type, DefaultDetectGroundCollisions>();
        ...
        private static Dictionary<Type, DefaultDetectLayers> s_DefaultDetectLayers = new Dictionary<Type, DefaultDetectLayers>();
        ...
        private static Dictionary<Type, DefaultMaxTriggerObjectCount> s_DefaultMaxTriggerObjectCount = new Dictionary<Type, DefaultMaxTriggerObjectCount>();
        ...
        public static Ability AddAbility(UltimateCharacterLocomotion characterLocomotion, Type abilityType, int index)
        {
            ...
            if (requiredComponents != null && requiredComponents.Length > 0) {
                for (int i = 0; i < requiredComponents.Length; ++i) {
                    if (characterLocomotion.gameObject.GetComponent(requiredComponents[i].m_Type0) == null) {
        ...
        public static ItemAbility AddItemAbility(UltimateCharacterLocomotion characterLocomotion, Type abilityType, int index)
        {
            ...
            itemAbilities[index] = itemAbility;
        ...
        public static void RemoveAbility(UltimateCharacterLocomotion characterLocomotion, Ability ability)
        {
            ...
            if (ability is ItemAbility itemAbility) {
                RemoveItemAbility(characterLocomotion, itemAbility);
                return;
            }
        ...
        /// </summary>
        /// <param name="characterLocomotion">The character to remove the ability from.</param>
        /// <param name="itemAbility">The item ability to remove.</param>
        public static void RemoveItemAbility(UltimateCharacterLocomotion characterLocomotion, ItemAbility itemAbility)
        {
            var itemAbilities = new ItemAbility[characterLocomotion.ItemAbilities.Length - 1];
            var index = 0;
            for (int i = 0; i < characterLocomotion.ItemAbilities.Length; ++i) {
                if (characterLocomotion.ItemAbilities[i] != itemAbility) {
                    itemAbilities[index] = characterLocomotion.ItemAbilities[i];
                    index++;
                }
            }
        ...
        private static void SetAbilityDefaultValues(Ability ability)
        {
            ...
            var defaultDetectGroundCollisions = GetDefaultDetectGroundCollisions(abilityType);
            if (defaultDetectGroundCollisions != null) {
                ability.DetectGroundCollisions = defaultDetectGroundCollisions.Value;
            }
            if (ability is DetectObjectAbilityBase) {
                ...
                var defaultDetectLayers = GetDefaultDetectLayers(abilityType);
                if (defaultDetectLayers != null) {
                    (ability as DetectObjectAbilityBase).DetectLayers = defaultDetectLayers.Value;
                }
                ...
                var defaultMaxTriggerObjectCount = GetDefaultMaxTriggerObjectCount(abilityType);
                if (defaultMaxTriggerObjectCount != null) {
                    (ability as DetectObjectAbilityBase).MaxTriggerObjectCount = defaultMaxTriggerObjectCount.Value;
                }
        ...
        /// <summary>
        /// Returns the DefaultDetectGroundCollisions of the specified ability type.
        /// </summary>
        /// <param name="type">The type of ability.</param>
        /// <returns>The DefaultDetectGroundCollisions of the specified ability type. Can be null.</returns>
        private static DefaultDetectGroundCollisions GetDefaultDetectGroundCollisions(Type type)
        {
            DefaultDetectGroundCollisions defaultDetectGroundCollisions;
            if (s_DefaultDetectGroundCollisions.TryGetValue(type, out defaultDetectGroundCollisions)) {
                return defaultDetectGroundCollisions;
            }

            if (type.GetCustomAttributes(typeof(DefaultDetectGroundCollisions), true).Length > 0) {
                defaultDetectGroundCollisions = type.GetCustomAttributes(typeof(DefaultDetectGroundCollisions), true)[0] as DefaultDetectGroundCollisions;
            }
            s_DefaultDetectGroundCollisions.Add(type, defaultDetectGroundCollisions);
            return defaultDetectGroundCollisions;
        }
        ...
        /// <summary>
        /// Returns the DefaultDetectLayers of the specified ability type.
        /// </summary>
        /// <param name="type">The type of ability.</param>
        /// <returns>The DefaultDefaultDetectLayersObjectDetection of the specified ability type. Can be null.</returns>
        private static DefaultDetectLayers GetDefaultDetectLayers(Type type)
        {
            DefaultDetectLayers defaultDetectLayers;
            if (s_DefaultDetectLayers.TryGetValue(type, out defaultDetectLayers)) {
                return defaultDetectLayers;
            }

            if (type.GetCustomAttributes(typeof(DefaultDetectLayers), true).Length > 0) {
                defaultDetectLayers = type.GetCustomAttributes(typeof(DefaultDetectLayers), true)[0] as DefaultDetectLayers;
            }
            s_DefaultDetectLayers.Add(type, defaultDetectLayers);
            return defaultDetectLayers;
        }
        ...
        /// <summary>
        /// Returns the DefaultMaxTriggerObjectCount of the specified ability type.
        /// </summary>
        /// <param name="type">The type of ability.</param>
        /// <returns>The DefaultMaxTriggerObjectCount of the specified ability type. Can be null.</returns>
        private static DefaultMaxTriggerObjectCount GetDefaultMaxTriggerObjectCount(Type type)
        {
            DefaultMaxTriggerObjectCount defaultMaxTriggerObjectCount;
            if (s_DefaultMaxTriggerObjectCount.TryGetValue(type, out defaultMaxTriggerObjectCount)) {
                return defaultMaxTriggerObjectCount;
            }

            if (type.GetCustomAttributes(typeof(DefaultMaxTriggerObjectCount), true).Length > 0) {
                defaultMaxTriggerObjectCount = type.GetCustomAttributes(typeof(DefaultMaxTriggerObjectCount), true)[0] as DefaultMaxTriggerObjectCount;
            }
            s_DefaultMaxTriggerObjectCount.Add(type, defaultMaxTriggerObjectCount);
            return defaultMaxTriggerObjectCount;
        }
