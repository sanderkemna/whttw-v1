                ...
                if (root.ParentId == 0 && root.PackageDownloader != null)
                {
                    ...
                    switch (state.state)
                    {
                        ...
                        case AssetDownloader.State.Unavailable:
                            if (root.PackageDownloader.IsDownloadSupported() && GUILayout.Button("Download")) root.PackageDownloader.Download();
                            break;

                        case AssetDownloader.State.Paused:
                            if (root.PackageDownloader.IsDownloadSupported() && GUILayout.Button("Resume Download" + (root.PackageSize > 0 ? $" ({EditorUtility.FormatBytes(root.PackageSize - root.PackageDownloader.GetState().bytesDownloaded)})" : ""))) root.PackageDownloader.Download();
                            break;

                        case AssetDownloader.State.UpdateAvailable:
                            if (root.PackageDownloader.IsDownloadSupported() && GUILayout.Button("Download Update" + (root.PackageSize > 0 ? $" ({EditorUtility.FormatBytes(root.PackageSize)})" : "")))
                            {
                    ...
                    if (info.AssetSource == Asset.Source.RegistryPackage)
                    {
                        ...
                        if (info.IsFeaturePackage())
                        {
                            UIBlock("package.actions.remove", () =>
                            {
                                ...
                                if (installed.Count > 0 && GUILayout.Button(UIStyles.Content($"Uninstall {installed.Count} Packages", "Remove installed packages out of this feature from current project.")))
                                {
