...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public class ItemSetGroup
    {
        ...
        {
            if (temporaryAssignedActiveItemSetPriority != SlotCount &&
                (activeItemSet == itemSet)
                && IsItemSetValid(setIndex, false, -1)) {
                temporaryAssignedActiveItemSet = setIndex;
                temporaryAssignedActiveItemSetPriority = SlotCount;
            }

            if (temporaryAssignedNextItemSetPriority != SlotCount &&
                (nextItemSet == itemSet)
                && IsItemSetValid(setIndex, false, -1)) {
            ...
            /*if (activeItemSet != null && temporaryAssignedActiveItemSetPriority < SlotCount) {
                var matchCount = -1;
                foreach (var activeItemIdentifier in activeItemSet.ItemIdentifiers) {
                    matchCount += itemSet.ItemIdentifiers.Contains(activeItemIdentifier) ? 1 : 0;
                }

                if (temporaryAssignedActiveItemSetPriority < matchCount
                    && IsItemSetValid(setIndex, false, -1)){
                    temporaryAssignedActiveItemSet = setIndex;
                    temporaryAssignedActiveItemSetPriority = matchCount;
                }
            }*/
            ...
            if (nextItemSet != null && temporaryAssignedNextItemSetPriority < SlotCount) {
                ...
                if (temporaryAssignedNextItemSetPriority < matchCount
                    && IsItemSetValid(setIndex, false, -1)) {
                    temporaryAssignedNextItemSet = setIndex;
