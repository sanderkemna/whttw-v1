        ...
        private void DrawSettingsTab()
        {
            ...
            if (AI.Config.showAdvancedSettings || ShowAdvanced())
            {
                ...
                if (AI.Config.showAdvancedSettings)
                {
                    ...
                    GUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(UIStyles.Content("Disable Drag & Drop", "Will not allow to drag & drop items from the search into other Unity views. Can improve selection behavior in case you are struggling with these in the search results."), EditorStyles.boldLabel, GUILayout.Width(labelWidth));
                    AI.Config.disableDragDrop = EditorGUILayout.Toggle(AI.Config.disableDragDrop, GUILayout.MaxWidth(cbWidth));
                    GUILayout.EndHorizontal();
